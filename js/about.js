(function ($) {
  ...
  function animateDemoFullscreen() {
    if (!$('.demographics3').hasClass('in-view')) {
      return;
    }
    ...
    // if ($('.demographics3').hasClass('in-view')) {  // scroll down abit and
    // get the action change this to AFTER states stats have appeared if
    ...
    // if($('.demographics4').hasClass('in-view')) {  // scroll down abit and
    // get the action
  ...
  function animateDemoMobile() {
    // mobile & tablet adjustments
    // us map
    if (!oneTimeAnimateElements.mobileStates && $('.state-percents-group').hasClass('in-view')) {
      oneTimeAnimateElements.mobileStates = true;
      $("#dots1")
      // .delay(00)
        .queue(function (next) {
          $(this).css('display', 'block');
    ...
    if (!oneTimeAnimateElements.mobileSpeaking && $('.field--name-field-students-speaking').hasClass('in-view')) {
      oneTimeAnimateElements.mobileSpeaking = true;
      $(".field--name-field-students-speaking .progress-bar")
        .delay(300)
        .queue(function (next) {
          var each_bar_width = $(this).attr('aria-valuenow');
    ...
    //"demographics" only complete in mobile if both "mobileStates" and
    // "mobileSpeaking" are both complete
    if (oneTimeAnimateElements.mobileStates && oneTimeAnimateElements.mobileSpeaking) {
      oneTimeAnimateElements.demographics = true;
    }
  }
